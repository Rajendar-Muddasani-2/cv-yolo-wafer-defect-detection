...
def _draw_wafer_background(
    img: Image.Image, draw: ImageDraw.ImageDraw, rng: np.random.Generator
) -> None:
    """Draw a realistic circular wafer with die grid, flat zone, and texture."""
    cx, cy = IMG_SIZE // 2, IMG_SIZE // 2
    radius = IMG_SIZE // 2 - _randint(rng, 15, 30)
    base_gray = _randint(rng, 170, 200)
    wafer_color = (base_gray, base_gray, base_gray + _randint(rng, 0, 15))
    ...
    # Die grid
    die_w = _randint(rng, 16, 28)
    die_h = _randint(rng, 16, 28)
    gc_base = base_gray - _randint(rng, 10, 25)
    for x in range(cx - radius, cx + radius, die_w):
        for y in range(cy - radius, cy + radius, die_h):
            if (x - cx) ** 2 + (y - cy) ** 2 < (radius - 5) ** 2:
                gc = max(0, min(255, gc_base + _randint(rng, -5, 6)))
...
def _add_scratch(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    x1 = _randint(rng, 40, IMG_SIZE - 40)
    y1 = _randint(rng, 40, IMG_SIZE - 40)
    angle = rng.uniform(-0.8, 0.8)
    length = _randint(rng, 60, 300)
    width = _randint(rng, 1, 6)
    x2 = int(x1 + length * math.cos(angle))
    y2 = int(y1 + length * math.sin(angle))
    segments = _randint(rng, 2, 5)
    points = [(x1, y1)]
    for s in range(1, segments):
        frac = s / segments
        points.append((int(x1 + (x2 - x1) * frac + _randint(rng, -8, 9)),
                   int(y1 + (y2 - y1) * frac + _randint(rng, -8, 9))))
    points.append((x2, y2))
    cv = _randint(rng, 30, 70)
...
def _add_particle(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 80, IMG_SIZE - 80), _randint(rng, 80, IMG_SIZE - 80)
    r = _randint(rng, 4, 25)
    glow_r = r + _randint(rng, 2, 6)
    gv = _randint(rng, 100, 140)
    draw.ellipse([cx - glow_r, cy - glow_r, cx + glow_r, cy + glow_r], fill=(gv, gv, gv))
    cv = _randint(rng, 20, 60)
...
def _add_edge_chip(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    angle = rng.uniform(0, 2 * math.pi)
    radius = IMG_SIZE // 2 - _randint(rng, 20, 40)
    cx = int(IMG_SIZE // 2 + radius * math.cos(angle))
    cy = int(IMG_SIZE // 2 + radius * math.sin(angle))
    size = _randint(rng, 12, 45)
    n_pts = _randint(rng, 4, 8)
    ...
    draw.polygon(points, fill=(_randint(rng, 50, 80),) * 3)
...
def _add_void(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 120, IMG_SIZE - 120), _randint(rng, 120, IMG_SIZE - 120)
    w, h = _randint(rng, 15, 55), _randint(rng, 15, 55)
    vv = _randint(rng, 15, 40)
...
def _add_pattern_shift(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 120, IMG_SIZE - 120), _randint(rng, 120, IMG_SIZE - 120)
    w, h = _randint(rng, 35, 90), _randint(rng, 35, 90)
    sx, sy = _randint(rng, 3, 8), _randint(rng, 3, 8)
    oc = (_randint(rng, 180, 220), _randint(rng, 60, 100), _randint(rng, 60, 100))
...
def _add_bridge(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 100, IMG_SIZE - 100), _randint(rng, 100, IMG_SIZE - 100)
    horiz = rng.random() > 0.5
    color = (_randint(rng, 200, 240), _randint(rng, 180, 220), _randint(rng, 100, 140))
    if horiz:
        length, height = _randint(rng, 20, 60), _randint(rng, 3, 8)
        draw.rectangle(
            [cx - length // 2, cy - height // 2, cx + length // 2, cy + height // 2],
            fill=color,
        )
        return 5, cx / IMG_SIZE, cy / IMG_SIZE, length / IMG_SIZE, max(height, 6) / IMG_SIZE
    else:
        h2, w2 = _randint(rng, 20, 60), _randint(rng, 3, 8)
        draw.rectangle([cx - w2 // 2, cy - h2 // 2, cx + w2 // 2, cy + h2 // 2], fill=color)
        return 5, cx / IMG_SIZE, cy / IMG_SIZE, max(w2, 6) / IMG_SIZE, h2 / IMG_SIZE


def _add_missing_bond(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 80, IMG_SIZE - 80), _randint(rng, 80, IMG_SIZE - 80)
    r = _randint(rng, 8, 20)
...
def _add_crack(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    sx = _randint(rng, 100, IMG_SIZE - 100)
    sy = _randint(rng, 100, IMG_SIZE - 100)
    points = [(sx, sy)]
    branches: List[List[Tuple[int, int]]] = [[(sx, sy)]]
    for _ in range(_randint(rng, 5, 15)):
        last = branches[0][-1]
        new_pt = (last[0] + _randint(rng, -20, 21), last[1] + _randint(rng, -20, 21))
        branches[0].append(new_pt)
        points.append(new_pt)
        if rng.random() < 0.3 and len(branches) < 4:
            branch = [new_pt]
            for _ in range(_randint(rng, 2, 5)):
                bpt = (branch[-1][0] + _randint(rng, -15, 16), branch[-1][1] + _randint(rng, -15, 16))
                branch.append(bpt)
                points.append(bpt)
            branches.append(branch)
    cv = _randint(rng, 30, 60)
    for br in branches:
        if len(br) >= 2:
            draw.line(br, fill=(cv, cv, cv), width=_randint(rng, 1, 3))
    xs, ys = [p[0] for p in points], [p[1] for p in points]
...
def _add_contamination(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 120, IMG_SIZE - 120), _randint(rng, 120, IMG_SIZE - 120)
    ax, ay = [], []
    for _ in range(_randint(rng, 3, 8)):
        ox, oy = cx + _randint(rng, -30, 31), cy + _randint(rng, -30, 31)
        rx, ry = _randint(rng, 10, 35), _randint(rng, 10, 35)
        cv = _randint(rng, 80, 120)
...
def _add_delamination(draw: ImageDraw.ImageDraw, rng: np.random.Generator) -> Tuple[int, float, float, float, float]:
    cx, cy = _randint(rng, 100, IMG_SIZE - 100), _randint(rng, 100, IMG_SIZE - 100)
    w, h = _randint(rng, 25, 70), _randint(rng, 25, 70)
    n_pts = _randint(rng, 6, 12)
    points = [(int(cx + w * rng.uniform(0.7, 1.0) * math.cos(2 * math.pi * i / n_pts)),
               int(cy + h * rng.uniform(0.7, 1.0) * math.sin(2 * math.pi * i / n_pts)))
              for i in range(n_pts)]
    draw.polygon(points, fill=(_randint(rng, 200, 230), _randint(rng, 190, 220), _randint(rng, 180, 210)))
    draw.polygon(points, outline=(_randint(rng, 80, 120), _randint(rng, 70, 110), _randint(rng, 60, 100)), width=2)
    xs, ys = [p[0] for p in points], [p[1] for p in points]
...
def _generate_single_image(
    ...
    n_defects = _randint(rng, 1, max_defects + 1)
    weights = np.ones(NUM_CLASSES) / NUM_CLASSES
    labels: List[str] = []
    for _ in range(n_defects):
        cls_idx = int(rng.choice(NUM_CLASSES, p=weights))
